...
namespace ARMEPlayback
{
    ...
    public class SimplePlaybackController : IDisposable
    {
        ...
        /// <summary>
        /// Test DLL loading - call this first to diagnose DLL issues
        /// </summary>
        public static bool TestDLLLoading(out string errorMessage)
        {
            try
            {
                // Try to call a simple function to test if DLL loads
                var testHandle = NativeMethods.arme_playback_create(44100, 512);
                if (testHandle != IntPtr.Zero)
                {
                    NativeMethods.arme_playback_destroy(testHandle);
                    errorMessage = "DLL loaded successfully!";
                    return true;
                }
                else
                {
                    errorMessage = "DLL loaded but function returned null - check library build";
                    return false;
                }
            }
            catch (DllNotFoundException ex)
            {
                errorMessage = $"DLL not found: {ex.Message}\n" +
                              "Check that ARMEPlaybackControllerLib.dll and dependencies are in correct folder:\n" +
                              "- Assets/Plugins/x86_64/ (for 64-bit Unity)\n" +
                              "- Assets/Plugins/x86/ (for 32-bit Unity)\n" +
                              "Required files: ARMEPlaybackControllerLib.dll, rubberband-3.dll, samplerate.dll, sleef.dll, sleefdft.dll";
                return false;
            }
            catch (BadImageFormatException ex)
            {
                errorMessage = $"Architecture mismatch: {ex.Message}\n" +
                              "Make sure DLL architecture matches Unity editor architecture.\n" +
                              "Check Edit > Project Settings > Player > Configuration";
                return false;
            }
            catch (Exception ex)
            {
