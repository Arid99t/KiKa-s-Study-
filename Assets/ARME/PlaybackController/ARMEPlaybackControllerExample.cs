...
namespace ARMEPlayback
{
    ...
    public class UnityPlaybackControllerExample : MonoBehaviour
    {
        ...
        [Tooltip("Automatically start playback when initialized")]
        ...
        private void InitializePlaybackController()
        {
            try
            {
                ...
                // Start the stretcher only if autoStart is enabled
                if (autoStart)
                {
                ...
                _isInitialized = true;
                Debug.Log($"🎵 Playback Controller ready! Auto-start: {autoStart}");
        ...
        public void StartPlayback()
        {
            if (_isInitialized && _playbackController != null)
            {
                ...
                {
                    Debug.LogWarning($"Failed to start playback: {ex.Message}");
                }
            }
        }
        ...
        public void StopPlayback()
        {
            if (_isInitialized && _playbackController != null)
            {
                ...
                {
                    Debug.LogWarning($"Failed to stop playback: {ex.Message}");
                }
            }
        }
        ...
        public bool IsPlaying
        {
            get
            {
                // Since the underlying library doesn't expose playing state,
                // we'll track this based on whether we've started vs stopped
                return _isInitialized && _playbackController != null;
            }
        }
        ...
        /// <summary>
        /// Reset playback to beginning without starting
        /// </summary>
        public void ResetPlayback()
        {
            if (_isInitialized && _playbackController != null)
            {
                try
                {
                    _playbackController.Stop();
                    _playbackController.SetAudioBuffer(_audioBuffer, _originalSampleRate);
                    Debug.Log("🔄 Playback reset to beginning");
                }
                catch (ARMEPlaybackException ex)
                {
                    Debug.LogWarning($"Failed to reset playback: {ex.Message}");
                }
            }
        }
